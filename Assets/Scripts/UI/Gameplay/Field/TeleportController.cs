...
public class TeleportController : MonoBehaviour
{
    ...
    private void Teleport()
    {
            ...
            )
        {
            ...
            //Ищем рандомный выход для текущего телепорта
            if (GameFieldCTRL.main.teleportLists[ID] != null) {
                for (int trying = 0; trying < GameFieldCTRL.main.teleportLists[ID].Count; trying++) {
                    ...
                    //Если обнаружилась дырка то пересоздаем список чтобы без дырки
                    if (GameFieldCTRL.main.teleportLists[ID][rand] == null) {
                        ...
                        //Новый список готов запомианем
                        GameFieldCTRL.main.teleportLists[ID] = teleportListNew;
                        //Выходим из цикла т.к. поменялась размерность списка
                        break;
                    }
                    ...
                    if (GameFieldCTRL.main.teleportLists[ID][rand].cellOut != null && //точка выхода существует
                        GameFieldCTRL.main.teleportLists[ID][rand].cellOut.Box == 0 && //Движение на выходе не заблокировано
                        !GameFieldCTRL.main.teleportLists[ID][rand].cellOut.cellInternal && //Место выхода не занято
                        GameFieldCTRL.main.teleportLists[ID][rand].cellOut.rock == 0 //на месте выхода нет камня
                                                         )
                    {
